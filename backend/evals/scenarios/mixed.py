...
scenario = Scenario(
    ...
    lines=[
        Line(
            "Sarah",
            "Alright let's do this. I finished the OAuth login yesterday and pushed it to review.",
        ),
        Line(
            "Marcus",
            "I'll review that this morning. My payment webhook fix got approved so I'm merging that now. Done.",
        ),
        Line(
            "Jordan",
            "Nice. I'm still blocked on the migration, same as yesterday. DBA hasn't responded.",
        ),
        Line("Sarah", "Okay. Today I'm picking up the rate limiting middleware."),
        Line(
            "Marcus",
            "Oh before I forget, we got a request from the product team to add dark mode support. We should track that.",
        ),
        Line(
            "Jordan",
            "Yeah let's add that to the backlog. Once I'm unblocked I'll finish the migration and then maybe grab the onboarding tooltip tour.",
        ),
    ],
    expected_actions=[
